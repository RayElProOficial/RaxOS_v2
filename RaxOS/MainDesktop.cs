...
namespace RaxOS
{
    internal class MainDesktop
    {
        private static readonly Bitmap bitmap = new Bitmap(10, 10,
        ...
        [ManifestResourceStream(ResourceName = "RaxOS.Images.test.bmp")]
        static byte[] byte2;

        private static readonly Bitmap bitmap2 = new Bitmap(byte2);
        ...
        public static void StartDesk()
        {
            ...
            try
            {
                ...
                canvas.DrawFilledRectangle(Color.WhiteSmoke, 0, 1080, 1080, 60);
                canvas.DrawImage(bitmap, 100, 150);
                canvas.DrawImage(bitmap2, 150, 200);
                canvas.Display();
            }
            catch (Exception e)
            {
